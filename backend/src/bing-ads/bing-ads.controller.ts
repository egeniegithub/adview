...
export class BingAdsController {
  ...
  @Get('/ObtainBingAdsData/:email/:token')
  ObtainBingAdsData(@Param('email') email: string,@Param('token') token:string) {
    return this.bingAdsService.ObtainBingAdsData(email,token);
  }
  @Post()
