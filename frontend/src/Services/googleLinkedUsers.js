...
const getActDetails  =async(list,token) =>{
  ...
  for (let i = 0; i < list.length; i++) {
    ...
    let arr = e.split("/");
    let id= arr[1]
    var data = {"query":"SELECT customer.id,customer_client.descriptive_name,customer_client.id, customer.manager,customer.status,customer_client.status, customer.resource_name, customer.descriptive_name FROM customer_client WHERE customer_client.manager != TRUE"};
    var config = {
          method: 'post',
